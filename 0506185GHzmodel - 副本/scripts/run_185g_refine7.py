"""run_185g_refine7.py — Run Round 7B (k=0.95 L2_g3 fine sweep). MAX_WORKERS=2."""
...
MANIFEST   = MODEL_DIR / "runs" / "refine7_manifest.json"
...
def update_log(results):
    ...
    for r in sorted(results, key=lambda x: x["L2_g3185g"]):
        m = r["metrics"]
        if m is None: rows.append(f"| {r['uid']} | {r['L2_g3185g']} | — | — | — | — | no s3p |"); continue
        ...
        rows.append(f"| {r['uid']} | {r['L2_g3185g']} | {fc:.3f} {okfc} | {m['S31_at_19GHz_dB']:.1f} {ok19} | {m['S31_at_20GHz_dB']:.1f} | {m['ripple_20_25GHz_dB']:.2f} | {m['min_IL_20plus_dB']:.1f} |{star}")
    ...
    if met:
        ...
        ref=f"k=0.95 方向找到更好结果。[{best['uid']}] L2_g3={best['L2_g3185g']} 全部达标，cross={m['crossing_GHz']:.3f}, S31@19={m['S31_at_19GHz_dB']:.1f} dB。"
        nxt="若 S31 余量 ≥1 dB，更新 final/，优化完成。若余量仍薄，结合 L7_g3 方向最佳参数做最终组合。"
        import shutil; s3p=Path(best["run_dir"])/"result.s3p"; fd=MODEL_DIR/"final"
        if s3p.exists(): shutil.copy(s3p, fd/f"diplexer_185g_{best['uid']}.s3p")
    else:
        ref="k=0.95 方向在当前 wC4/wC2 下，所有 L2_g3 值均未同时满足目标。k 的微小变化不足以突破现有瓶颈。"
        nxt="以 034 为最终方案（S31@19=−10.2 dB 达标，余量薄但可接受），或考虑更大范围的 k+wC4+L2_g3 联合搜索。"
    section = f"""
...
"""
    ...
    print("TUNING_LOG updated (refine7).")
...
def main():
    ...
    for c in cases:
        ...
        if open_html: make_html(c["run_dir"],fr,s11,s21,s31,m, f"[{c['uid']}] k=0.95 L2_g3={c['L2_g3185g']}",open_it=True)
    HDR = f"{'UID':>4} {'L2g3':>5} {'Cross':>7} {'S31@19':>8} {'S31@20':>8} {'Ripple':>7} {'IL':>6}  Targets"
    SEP = "-"*len(HDR)
    print(HDR); print(SEP)
    print(f"{'034':>4} {'0.78':>5} {'18.540':>7} {'-10.2':>8} {'-12.3':>8} {'0.58':>7} {'-1.2':>6}  ✓✓ k=0.96 ref")
    print(SEP)
    for r in sorted(results, key=lambda x: x["L2_g3185g"]):
        m=r["metrics"]
        if m is None: print(f"{r['uid']:>4} {r['L2_g3185g']:>5.2f}  (no s3p)"); continue
        fc=m["crossing_GHz"]; okfc="✓" if fc and fc>=18.5 else "✗"; ok19="✓" if m["S31_at_19GHz_dB"]<=-10.0 else "✗"
        star=" ★" if (fc and fc>=18.5 and m["S31_at_19GHz_dB"]<=-10.0) else ""
        fc_s = f"{fc:.3f}" if fc else "  n/a "
        print(f"{r['uid']:>4} {r['L2_g3185g']:>5.2f} {fc_s:>7}"
