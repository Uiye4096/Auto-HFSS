"""
build_185g_refine6.py — Round 7A: sweep L7_g3185g at base 034.

L7_g3185g is unique to the 185g topology (absent in 12g model).
Hypothesis: it is a shunt resonator in the LPF that creates a transmission zero.
Sweeping its length tunes the zero frequency relative to 19 GHz.

Base: 034 (k=0.96, wC4=1.15, wC2=1.08, L4_g3=0.85, L2_g3=0.78)
Sweep L7_g3185g: [0.70, 0.75, 0.80, 0.85, 0.95, 1.00] → UIDs 036-041
"""
import json
import subprocess
import sys
...
L7G3_VALS = [0.70, 0.75, 0.80, 0.85, 0.95, 1.00]
...
def main():
    ...
    print(f"Building {len(L7G3_VALS)} cases (L7_g3 sweep)  base: 034")
    print(f"  L7_g3185g: {L7G3_VALS}  (baseline 0.90)\n")
    ...
    mp = MODEL_DIR / "runs" / "refine6_manifest.json"
