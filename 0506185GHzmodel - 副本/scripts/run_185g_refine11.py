"""run_185g_refine11.py — Round 11: L7_g4185g + l_line2185g S11 sweep."""
...
MANIFEST   = MODEL_DIR / "runs" / "refine11_manifest.json"
TUNING_LOG = MODEL_DIR / "TUNING_LOG.md"
MAX_WORKERS = 4
F_MAX = 30.0

REF024 = {"uid":"024","crossing_GHz":18.535,"S31_at_19GHz_dB":-10.1,
          "S31_at_20GHz_dB":-11.9,"ripple_20_25GHz_dB":0.56,
          "min_IL_20plus_dB":-1.3,"S11_worst_19_25GHz_dB":-7.2,
...
def update_log(results):
    def row(r):
        m = r["metrics"]
        if m is None: return f"| {r['uid']} | {r['direction']} | {r['param']}={r['value']} | — | — | — | — | no s3p |"
        fc = m["crossing_GHz"]; fc_s = f"{fc:.3f}" if fc else "n/a"
        ok_fc="✓" if fc and fc>=18.5 else "✗"; ok_19="✓" if m["S31_at_19GHz_dB"]<=-10.0 else "✗"
        ok_s11="✓" if m["S11_worst_19_25GHz_dB"]<=-10.0 else "✗"
        star=" ★" if (fc and fc>=18.5 and m["S31_at_19GHz_dB"]<=-10.0 and m["ripple_20_25GHz_dB"]<=1.0 and m["S11_worst_19_25GHz_dB"]<=-10.0) else ""
        return (f"| {r['uid']} | {r['direction']} | {r['param']}={r['value']} | {fc_s} {ok_fc} | "
                f"{m['S31_at_19GHz_dB']:.1f} {ok_19} | {m['ripple_20_25GHz_dB']:.2f} | {m['S11_worst_19_25GHz_dB']:.1f} {ok_s11} |{star}")
    ...
    if best_s11:
        ...
        improved = delta < 0
        ref_str = (f"{best_s11['direction']}方向最优：{best_s11['uid']} ({best_s11['param']}={best_s11['value']}mm), "
                   f"S11w={bm['S11_worst_19_25GHz_dB']:.1f} dB "
                   f"({'改善 ' + str(abs(delta)) + ' dB ✓' if improved else '未改善，与 024 持平'})")
        nxt = (f"以 {best_s11['uid']} 为基点精细扫参（±0.05mm 步长）。" if improved else
               "两个方向均未改善 S11。高频 S11 可能需要修改 HPF 端口匹配网络几何，考虑同时扫多个参数。")
    else:
        ref_str = "无满足所有约束的 case 改善 S11。"
        nxt = "需重新评估 S11 改善策略，可能需要拓扑修改。"

    rows = [row(r) for r in results]
    section = f"""
## Round 11 — refine11：L7_g4185g + l_line2185g 扫参（S11 双向探索）

**基准**：024  S11w = {ref['S11_worst_19_25GHz_dB']:.1f} dB（@20=-12.1, @25=-5.8, @28=-4.6 dB）
...
"""
    ...
    print("TUNING_LOG updated (refine11).")
...
def main():
    ...
    HDR = f"{'UID':>4} {'Dir':>3} {'Param+Val':<22} {'Cross':>7} {'S31@19':>7} {'Rpl25':>6} {'S11w':>6} {'@20':>6} {'@25':>6} {'@28':>6}"
    print(HDR); print("-"*len(HDR))
    r = REF024
    print(f"{'024':>4} {'—':>3} {'baseline':22} {r['crossing_GHz']:>7.3f} {r['S31_at_19GHz_dB']:>7.1f} {r['ripple_20_25GHz_dB']:>6.2f} {r['S11_worst_19_25GHz_dB']:>6.1f} {r['S11_at_20GHz_dB']:>6.1f} {r['S11_at_25GHz_dB']:>6.1f} {r['S11_at_28GHz_dB']:>6.1f}  (ref)")
    print("-"*len(HDR))

    results = []
    for c in sorted(cases, key=lambda x: (x["direction"], x["value"])):
        s3p = Path(c["run_dir"]) / "result.s3p"
        if not s3p.exists(): results.append({**c, "metrics": None}); print(f"{c['uid']:>4}  no s3p"); continue
        ...
        ok_s11 = "✓" if m["S11_worst_19_25GHz_dB"] <= -10.0 else "✗"
        star = " ★" if (fc and fc>=18.5 and m["S31_at_19GHz_dB"]<=-10.0 and m["ripple_20_25GHz_dB"]<=1.0 and m["S11_worst_19_25GHz_dB"]<=-10.0) else ""
        tag = f"{c['param']}={c['value']}"
        print(f"{c['uid']:>4} {c['direction']:>3} {tag:<22} {fc_s:>7} {m['S31_at_19GHz_dB']:>7.1f}{ok19} {m['ripple_20_25GHz_dB']:>6.2f} {m['S11_worst_19_25GHz_dB']:>6.1f}{ok_s11} {m['S11_at_20GHz_dB']:>6.1f} {m['S11_at_25GHz_dB']:>6.1f} {m['S11_at_28GHz_dB']:>6.1f}{star}")
        make_html(c["run_dir"], fr, s11, s21, s31, m,
                  f"[{c['uid']}] {c['direction']} {tag}  S11w={m['S11_worst_19_25GHz_dB']:.1f}dB")
