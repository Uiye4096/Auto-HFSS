"""
run_185g_refine1.py — Run and analyse Phase 2 k-sweep cases.

Usage:
    python scripts/run_185g_refine1.py              # run all + analyse
    python scripts/run_185g_refine1.py --analyse    # re-analyse only
    python scripts/run_185g_refine1.py --html       # generate + open HTML viewers
"""
import json
import math
import subprocess
...
MANIFEST  = MODEL_DIR / "runs" / "refine1_manifest.json"
MAX_WORKERS = 4
...
def main():
    ...
    if not MANIFEST.exists():
        sys.exit("Run build_185g_refine1.py first.")
    ...
    BASELINE_002 = {"crossing_GHz": 18.381, "S31_at_19GHz_dB": -10.3,
                    "S31_at_20GHz_dB": -10.8, "ripple_20_25GHz_dB": 0.84,
                    "min_IL_20plus_dB": -1.6, "S11_worst_19_25GHz_dB": -6.9}
    ...
    for c in cases:
        ...
        if open_html:
            make_html(c["run_dir"], fr, s11, s21, s31, m,
                      f"185g refine1 [{c['uid']}] k={c['k']}  wC4={c['w_C4']} wC2={c['w_C2']}",
                      open_it=True)

    HDR = f"{'UID':>4} {'k':>5} {'Cross':>7} {'S31@19':>8} {'S31@20':>8} {'Ripple':>7} {'IL':>6} {'S11w':>6}  Targets"
    SEP = "-" * len(HDR)
    print(HDR)
    print(SEP)
    ...
    def fmt_row(uid, k, m):
        if m is None:
            return f"{uid:>4} {k:>5.2f}  {'(no s3p)':>50}"
        fc   = m["crossing_GHz"]
        ...
        return (f"{uid:>4} {k:>5.2f} {fc_s:>7} {m['S31_at_19GHz_dB']:>8.1f}"
                f" {m['S31_at_20GHz_dB']:>8.1f} {m['ripple_20_25GHz_dB']:>7.2f}"
                f" {m['min_IL_20plus_dB']:>6.1f} {m['S11_worst_19_25GHz_dB']:>6.1f}"
                f"  {okfc}cross {ok19}S31@19 {ok20}S31@20 {okrp}ripple")

    print(fmt_row("002", 1.00, BASELINE_002) + "  ← Phase1 best (k=1.0)")
    print(SEP)
    for r in sorted(results, key=lambda x: x["k"]):
        print(fmt_row(r["uid"], r["k"], r["metrics"]))
    ...
    if met:
        ...
        print(f"Best: [{best['uid']}] k={best['k']}  "
