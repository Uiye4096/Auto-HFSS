"""run_185g_refine16.py — Round 16: L2_g3 sweep on 102 base."""
...
MANIFEST   = MODEL_DIR / "runs" / "refine16_manifest.json"
TUNING_LOG = MODEL_DIR / "TUNING_LOG.md"
MAX_WORKERS = 4
F_MAX = 30.0

REF102 = {"uid":"102","L4_g3185g":0.75,"L2_g3185g":0.78,
          "crossing_GHz":18.638,"S31_at_19GHz_dB":-7.1,"S31_at_20GHz_dB":-11.7,
          "ripple_20_25GHz_dB":0.54,"S11_worst_19_25GHz_dB":-9.7}
...
def update_log(results):
    ...
    for r in sorted(results, key=lambda x: x["L2_g3185g"]):
        m = r["metrics"]
        if m is None: rows.append(f"| {r['uid']} | {r['L2_g3185g']:.2f} | — | — | — | — | no s3p |"); continue
        ...
        rows.append(f"| {r['uid']} | {r['L2_g3185g']:.2f} | {fc_s} {ok_fc} | {m['S31_at_19GHz_dB']:.1f} | {m['S31_at_20GHz_dB']:.1f} | {m['ripple_20_25GHz_dB']:.2f} | {m['S11_worst_19_25GHz_dB']:.1f} {ok_s11} |")
    ...
    if best:
        bm = best["metrics"]
        delta = round(bm["S11_worst_19_25GHz_dB"] - REF102["S11_worst_19_25GHz_dB"], 1)
        broke = bm["S11_worst_19_25GHz_dB"] <= -10.0
        ref_str = (f"突破 ★！{best['uid']} (L2_g3={best['L2_g3185g']:.2f}mm): S11w={bm['S11_worst_19_25GHz_dB']:.1f} dB ≤ −10 dB！" if broke else
                   f"最优：{best['uid']} (L2_g3={best['L2_g3185g']:.2f}mm) S11w={bm['S11_worst_19_25GHz_dB']:.1f} dB（vs 102 {delta:+.1f} dB）。")
        nxt = ("确立为 final，S11 目标全达标！" if broke else
...
"""
    ...
    print("TUNING_LOG updated (refine16).")
        ...
        if s3p.exists():
            ...
            (MODEL_DIR/"final"/"parameters.json").write_text(
                json.dumps({"uid": best["uid"], "metrics": best["metrics"],
                            "L4_g3185g": 0.75, "L2_g3185g": best["L2_g3185g"],
...
def main():
    ...
    HDR = f"{'UID':>4} {'L2g3':>5} {'Cross':>7} {'S31@19':>7} {'S31@20':>7} {'Rpl25':>6} {'S11w':>6} {'@20':>6} {'@25':>6} {'@28':>6}"
    print(HDR); print("-"*len(HDR))
    r = REF102
    print(f"{'102':>4} {'0.78':>5} {r['crossing_GHz']:>7.3f} {r['S31_at_19GHz_dB']:>7.1f} {r['S31_at_20GHz_dB']:>7.1f} {r['ripple_20_25GHz_dB']:>6.2f} {r['S11_worst_19_25GHz_dB']:>6.1f}  (base)")
    print("-"*len(HDR))

    results = []
    for c in sorted(cases, key=lambda x: x["L2_g3185g"]):
        ...
        print(f"{c['uid']:>4} {c['L2_g3185g']:>5.2f} {fc_s:>7}"
        ...
        make_html(c["run_dir"], fr, s11, s21, s31, m,
                  f"[{c['uid']}] L4g3=0.75 L2g3={c['L2_g3185g']:.2f}  S11w={m['S11_worst_19_25GHz_dB']:.1f}dB")
    ...
    if winners:
        ...
        for w in sorted(winners, key=lambda x: x["metrics"]["S11_worst_19_25GHz_dB"]):
            print(f"    [{w['uid']}] L2g3={w['L2_g3185g']:.2f}  S11w={w['metrics']['S11_worst_19_25GHz_dB']:.1f}")
        print(f"{'='*60}")
