"""
Run refine18 with parallel HFSS execution.

Launches up to MAX_WORKERS HFSS instances simultaneously.
Adjust MAX_WORKERS to match your license count.
"""
import json
import math
import subprocess
...
OUT_ROOT = ROOT / "12GHzdiplexer2" / "derived" / "refine18"
...
MAX_WORKERS = 4   # adjust to your HFSS license count
...
def run_one(item):
    ...
    if s3p.exists():
        with print_lock:
            print(f"  [cached] {item['case']}")
        return item, True
    with print_lock:
        print(f"  [start ] {item['case']}")
    ret = subprocess.run(
    ...
    with print_lock:
        print(f"  [{'done ' if ok else 'FAIL '}] {item['case']}")
    if ok:
        subprocess.run(
...
def main():
    ...
    # filter already-done cases first (instant), then sim the rest in parallel
    ...
    # ── Analysis ──────────────────────────────────────────────────────────────
    print("\n\n" + "="*72)
    print("RESULTS TABLE  (w_sub=1.7mm, L4=1.10mm, L2=0.75mm, idx=0.6)")
    print(f"  {'Case':<32} {'Cross':>8} {'S31':>7} {'worstIL':>9} {'ripple':>8}")
    print("-"*65)
    ...
    for it in manifest:
        ...
        upd = it["updates"]
        rows.append((upd["w_C412g"], upd["w_C1_y"], fc, lpf, hil, rip,
                     fr, s11, s21, s31, case))

    rows.sort(key=lambda x: (x[0], x[1]))
    for wc, c1y, fc, lpf, hil, rip, fr, s11, s21, s31, case in rows:
        ok_c  = "✓" if fc  and fc  >= 18.5 else " "
        ok_s  = "✓" if lpf and lpf >= 18.5 else " "
        fc_s  = f"{fc:.3f}"  if fc  else " none"
        lpf_s = f"{lpf:.2f}" if lpf else " FAIL"
        print(f"  {case:<32} {fc_s:>8}{ok_c} {lpf_s:>7}{ok_s} {hil:>9.1f} {rip:>8.1f}")

    # ── Top picks ─────────────────────────────────────────────────────────────
    print("\nTop picks (crossing>=18.5 AND best worst-IL):")
    good = [(hil, rip, case, fc, lpf, fr, s21, s31, s11)
            for wc, c1y, fc, lpf, hil, rip, fr, s11, s21, s31, case in rows
            if fc and fc >= 18.5]
    good.sort()
    for hil, rip, case, fc, lpf, fr, s21, s31, s11 in good[:5]:
        lpf_s = f"{lpf:.2f}" if lpf else "FAIL"
        ok_s  = "✓" if lpf and lpf >= 18.5 else " "
        print(f"  {case:<32}  cross={fc:.3f}  S31={lpf_s}{ok_s}"
              f"  IL={hil:.1f}  rip={rip:.1f}")
        print(f"    S21: " + "  ".join(f"@{f}={at(fr,s21,f):.1f}" for f in [19,20,21,22,25,28]))
