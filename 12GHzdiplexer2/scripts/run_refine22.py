"""Run refine22 — Phase B-2: L2_g312g sweep. Parallel execution."""
import json
import math
import subprocess
import threading
...
OUT_ROOT = ROOT / "12GHzdiplexer2" / "derived" / "refine22"
...
def main():
    ...
    # ── Results table ─────────────────────────────────────────────────────────
    print("\n" + "=" * 82)
    print("PHASE B-2 RESULTS  (w_C=1.4mm, bump suppressed, L2 sweep)")
    print(f"  {'Case':<32} {'L2':>6} {'Cross':>8} {'S31-3dB':>8} "
          f"{'Bump':>7} {'wrstIL':>8} {'rip':>6}")
    print("-" * 76)
    ...
    for it in manifest:
        ...
        l2 = it["updates"].get("L2_g312g", "?")
        l4 = it["updates"].get("L4_g312g", "?")
        rows.append((l4, l2, case, fc, lpf, hil, rip, f_bk, v_bk,
                     fr, s11, s21, s31))

    rows.sort(key=lambda x: (x[0], x[1]))
    for l4, l2, case, fc, lpf, hil, rip, f_bk, v_bk, *_ in rows:
        ...
        print(f"  {case:<32} {l2:>6}  {fc_s:>8}{ok_c} {lpf_s:>8} "
              f"{bk_s:>7}{ok_b} {hil:>8.1f} {rip:>6.1f}")

    # ── Best crossing with bump OK ─────────────────────────────────────────────
    print("\nBest crossing (bump≤-10 dB):")
    bump_ok = [(fc or 0, case, fc, lpf, hil, v_bk, l2, fr, s21, s31, s11)
               for l4, l2, case, fc, lpf, hil, rip, f_bk, v_bk, fr, s11, s21, s31
               in rows if v_bk is not None and v_bk <= -10.0]
    bump_ok.sort(reverse=True)
    for _, case, fc, lpf, hil, v_bk, l2, fr, s21, s31, s11 in bump_ok[:5]:
        ok_c = "✓" if fc and fc >= 18.5 else " "
        lpf_s = f"{lpf:.2f}" if lpf else "FAIL"
        print(f"  {case:<32}  L2={l2}  cross={fc:.3f}{ok_c}  "
              f"S31={lpf_s}  bump={v_bk:.1f}✓  IL={hil:.1f}")
        print(f"    S21: " + "  ".join(f"@{f}={at(fr,s21,f):.1f}"
                                       for f in [18, 19, 20, 21, 22, 25, 28]))
        print(f"    S31: " + "  ".join(f"@{f}={at(fr,s31,f):.1f}"
                                       for f in [17, 18, 19, 20, 21]))
