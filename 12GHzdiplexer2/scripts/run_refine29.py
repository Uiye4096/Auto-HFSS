"""Run refine29 — reference-guided exploration. Parallel execution."""
import json
import math
import subprocess
import threading
...
OUT_ROOT = ROOT / "12GHzdiplexer2" / "derived" / "refine29"
...
def main():
    ...
    # ── Results table ─────────────────────────────────────────────────────────
    print("\n" + "=" * 92)
    print("REFINE29  (reference-guided: 185g non-uniform LPF + g4 stub sweep)")
    ...
    # ── Winners full detail ────────────────────────────────────────────────────
    ref_row = [("r27_c1y022[best]", 18.552, -10.8, -15.8, -3.1, 0.6, -3.9)]
    ...
    print(f"\n\nPareto (cross≥18.5, S31@19≤-10, S31@20≤-10) vs prior best r27_c1y022:")
    print(f"  prior: cross=18.552✓  S31@19=-10.8✓  S31@20=-15.8✓  rip25=0.6✓  IL=-3.1  S11=-3.9")
    for rip25, case, fc, s19, s20, hil, s11w, fr, s11, s21, s31 in winners[:6]:
        ok_r = "✓" if rip25 <= 1.0 else " "
        print(f"\n  {case}  cross={fc:.3f}✓  S31@19={s19:.1f}✓  S31@20={s20:.1f}✓  "
              f"rip25={rip25:.1f}{ok_r}  IL={hil:.1f}  S11={s11w:.1f}")
    ...
    if not winners:
        print("\n  (none — showing best 4 by crossing):")
        for case, fc, s19, s20, hil, rip25, s11w, fr, s11, s21, s31 in rows[:4]:
            ok_c  = "✓" if fc and fc >= 18.5 else " "
            ok_19 = "✓" if s19 <= -10 else " "
            print(f"\n  {case}  cross={fc:.3f}{ok_c}  S31@19={s19:.1f}{ok_19}  rip25={rip25:.1f}")
            print(f"    S21: " + "  ".join(f"@{f}={at(fr,s21,f):.1f}"
                                           for f in [18,19,20,21,22,23,25,28,32]))
            print(f"    S31: " + "  ".join(f"@{f}={at(fr,s31,f):.1f}"
                                           for f in [17,18,19,20,21,22]))
