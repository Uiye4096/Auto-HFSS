"""Run refine8: solve + SVG plot + detailed dip/S11 analysis."""
import json
import math
import subprocess
...
OUT_ROOT = ROOT / "12GHzdiplexer2" / "derived" / "refine8"
...
def main():
    ...
    print(f"\n{'Case':<28} {'Cross':>8} {'S11@21':>8} {'S21@21':>8} {'S21@22':>8} {'S31-3dB':>8}")
    print("-" * 72)
    for item in manifest:
        print(f"\n{'='*60}\nCase: {item['case']}")
        name_pre = Path(item["project_path"]).stem
        s3p_pre  = Path(item["output_dir"]) / f"{name_pre}.s3p"
        if s3p_pre.exists():
            print("  (already done, reading results)")
            fr, s11, s21, s31 = parse_s3p(s3p_pre)
            ...
            print(f"  >> crossing={fc_s} GHz  S11@21={at(fr,s11,21):.1f}  "
                  f"S21@21={at(fr,s21,21):.1f}  S21@22={at(fr,s21,22):.1f}  S31-3dB={lpf_s}")
            continue
        ...
        print(f"  >> crossing={fc_s} GHz  "
              f"S11@21={at(fr,s11,21):.1f}  S21@21={at(fr,s21,21):.1f}  "
              f"S21@22={at(fr,s21,22):.1f}  S31-3dB={lpf_s}")
    print("\nAll done.")
