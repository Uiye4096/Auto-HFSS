"""Run refine11 with full passband integrity report."""
import json
import math
import subprocess
...
OUT_ROOT = ROOT / "12GHzdiplexer2" / "derived" / "refine11"
...
def report(name, fr, s11, s21, s31):
    ...
    # HPF passband max IL (worst insertion loss in 20-35 GHz)
    hpf_il = min(at(fr, s21, f) for f in [20, 21, 22, 23, 25, 28, 30, 35])
    fc_s  = f"{fc:.3f}" if fc else "none"
    lpf_s = f"{lpf:.2f}" if lpf else "FAIL"
    hpf_s = f"{hpf:.2f}" if hpf else "FAIL"
    ok_cross = "✓" if fc and fc >= 18.5 else "✗"
    ok_s31   = "✓" if lpf and lpf >= 18.5 else "✗"
    print(f"\n  ── {name} ──")
    print(f"  Crossing  : {fc_s} GHz {ok_cross}  |  S31 -3dB: {lpf_s} GHz {ok_s31}")
    print(f"  S21 edge  : {hpf_s} GHz  |  HPF passband worst IL (20-35GHz): {hpf_il:.1f} dB")
    print(f"  S21: @18={at(fr,s21,18):.1f} @19={at(fr,s21,19):.1f} @20={at(fr,s21,20):.1f}"
          f" @21={at(fr,s21,21):.1f} @22={at(fr,s21,22):.1f} @25={at(fr,s21,25):.1f}"
          f" @30={at(fr,s21,30):.1f} @35={at(fr,s21,35):.1f}")
    print(f"  S31: @16={at(fr,s31,16):.1f} @17={at(fr,s31,17):.1f} @18={at(fr,s31,18):.1f}"
          f" @19={at(fr,s31,19):.1f} @20={at(fr,s31,20):.1f}")
    print(f"  S11: @19={at(fr,s11,19):.1f} @20={at(fr,s11,20):.1f} @21={at(fr,s11,21):.1f}"
          f" @22={at(fr,s11,22):.1f}")
