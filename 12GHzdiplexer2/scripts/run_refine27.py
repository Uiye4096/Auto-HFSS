"""Run refine27 — Phase D final polish. Parallel execution."""
import json
import math
import subprocess
import threading
...
OUT_ROOT = ROOT / "12GHzdiplexer2" / "derived" / "refine27"
...
def metrics(fr, s11, s21, s31):
    ...
    s19   = at(fr, s31, 19.0)
    s20   = at(fr, s31, 20.0)
    # passband metrics from 20-32 GHz (above transition)
    pb    = [20, 21, 22, 23, 25, 28, 32]
    pb_v  = [at(fr, s21, f) for f in pb]
    hil   = min(pb_v)
    rip   = max(pb_v) - hil
    # S11 worst in HPF passband
    s11_w = min(at(fr, s11, f) for f in [19, 20, 21, 22, 25])
    return fc, s19, s20, hil, rip, s11_w
...
def main():
    ...
    # ── Summary table ─────────────────────────────────────────────────────────
    print("\n" + "=" * 84)
    print("REFINE27  Phase D — final polish  (base: k=0.90, wC=1.45, L4=0.85)")
    print(f"  {'Case':<28} {'Cross':>8} {'S31@19':>7} {'S31@20':>7} "
          f"{'IL@20+':>7} {'rip20+':>7} {'S11min':>7}")
    print("-" * 78)
    ...
    for it in manifest:
        ...
        fc, s19, s20, hil, rip, s11w = metrics(fr, s11, s21, s31)
        rows.append((case, fc, s19, s20, hil, rip, s11w, fr, s11, s21, s31))

    rows.sort(key=lambda x: -(x[1] or 0))
    for case, fc, s19, s20, hil, rip, s11w, *_ in rows:
        ok_c  = "✓" if fc  and fc  >= 18.5  else " "
        ok_19 = "✓" if s19 <= -10.0 else " "
        ok_20 = "✓" if s20 <= -10.0 else " "
        ok_r  = "✓" if rip <= 1.0   else " "
        fc_s  = f"{fc:.3f}" if fc else " none"
        print(f"  {case:<28} {fc_s:>8}{ok_c} {s19:>6.1f}{ok_19} {s20:>6.1f}{ok_20} "
              f"{hil:>6.1f} {rip:>6.1f}{ok_r} {s11w:>6.1f}")

    # ── Full detail for Pareto-front candidates ────────────────────────────────
    print("\n\nPareto front — all criteria: crossing≥18.5, S31@19≤-10, S31@20≤-10:")
    pareto = [(rip, hil, case, fc, s19, s20, s11w, fr, s11, s21, s31)
              for case, fc, s19, s20, hil, rip, s11w, fr, s11, s21, s31 in rows
              if fc and fc >= 18.5 and s19 <= -10.0 and s20 <= -10.0]
    pareto.sort()

    label = "Best ripple first:" if pareto else "(none — showing best trade-offs):"
    print(f"\n  {label}")

    if not pareto:
        pareto = sorted(
            [(rip, hil, case, fc, s19, s20, s11w, fr, s11, s21, s31)
             for case, fc, s19, s20, hil, rip, s11w, fr, s11, s21, s31 in rows
             if fc],
            key=lambda x: abs((x[3] or 0) - 18.5) - min(0, x[4]+10) - min(0, x[5]+10)
        )[:6]

    for entry in pareto[:6]:
        rip, hil, case, fc, s19, s20, s11w, fr, s11, s21, s31 = entry
        ok_c  = "✓" if fc >= 18.5 else " "
        ok_19 = "✓" if s19 <= -10 else " "
        ok_20 = "✓" if s20 <= -10 else " "
        print(f"\n  {case}  cross={fc:.3f}{ok_c}  S31@19={s19:.1f}{ok_19}  "
              f"S31@20={s20:.1f}{ok_20}  rip={rip:.1f}")
