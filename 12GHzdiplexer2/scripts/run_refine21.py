"""Run refine21 — Phase B: re-tune HPF crossing. Parallel execution."""
import json
import math
import subprocess
import threading
...
OUT_ROOT = ROOT / "12GHzdiplexer2" / "derived" / "refine21"
...
def s31_max_stopband(fr, s31):
    """Max S31 in 18-21 GHz (the bump region)."""
...
def metrics(fr, s11, s21, s31):
    ...
    hil  = min(vals)
    rip  = max(vals) - hil
    f_bk, v_bk = s31_max_stopband(fr, s31)
...
def main():
    ...
    # ── Results table ─────────────────────────────────────────────────────────
    print("\n" + "=" * 82)
    print("PHASE B RESULTS  (goal: crossing≥18.5 + S31_stop≤-10dB + IL as flat as possible)")
    print(f"  {'Case':<36} {'Cross':>8} {'S31-3dB':>8} {'Bump':>7} {'wrstIL':>8} {'rip':>6}")
    print("-" * 78)
    ...
    for case, fc, lpf, hil, rip, f_bk, v_bk, *_ in rows:
        ...
        print(f"  {case:<36} {fc_s:>8}{ok_c} {lpf_s:>8} "
              f"{bk_s:>7}{ok_b} {hil:>8.1f} {rip:>6.1f}")

    # ── Top picks ──────────────────────────────────────────────────────────────
    print("\nTop picks (crossing≥18.5 AND bump≤-10dB, sorted by worst-IL):")
    good = [(hil, rip, case, fc, lpf, v_bk, fr, s21, s31, s11)
            for case, fc, lpf, hil, rip, f_bk, v_bk, fr, s11, s21, s31, it in rows
            if fc and fc >= 18.5 and v_bk is not None and v_bk <= -10.0]
    good.sort()
    if not good:
        print("  (none met both criteria — showing closest to 18.5 GHz crossing)")
        good2 = sorted([(abs((fc or 0) - 18.5), case, fc, lpf, hil, v_bk, fr, s21, s31, s11)
                        for case, fc, lpf, hil, rip, f_bk, v_bk, fr, s11, s21, s31, it in rows
                        if fc], key=lambda x: x[0])
        for _, case, fc, lpf, hil, v_bk, fr, s21, s31, s11 in good2[:6]:
            lpf_s = f"{lpf:.2f}" if lpf else "FAIL"
            bk_s  = f"{v_bk:.1f}" if v_bk is not None else "---"
            print(f"  {case:<36}  cross={fc:.3f}  S31={lpf_s}  bump={bk_s}  IL={hil:.1f}")
            print(f"    S21: " + "  ".join(f"@{f}={at(fr,s21,f):.1f}" for f in [18,19,20,21,22,25]))
            print(f"    S31: " + "  ".join(f"@{f}={at(fr,s31,f):.1f}" for f in [18,19,20,21]))
    else:
        for hil, rip, case, fc, lpf, v_bk, fr, s21, s31, s11 in good[:6]:
            lpf_s = f"{lpf:.2f}" if lpf else "FAIL"
            bk_s  = f"{v_bk:.1f}" if v_bk is not None else "---"
            ok_s  = "✓" if lpf and lpf >= 18.5 else " "
            print(f"  {case:<36}  cross={fc:.3f}✓  S31={lpf_s}{ok_s}  bump={bk_s}✓  IL={hil:.1f}  rip={rip:.1f}")
            print(f"    S21: " + "  ".join(f"@{f}={at(fr,s21,f):.1f}" for f in [18,19,20,21,22,25,28]))
            print(f"    S31: " + "  ".join(f"@{f}={at(fr,s31,f):.1f}" for f in [17,18,19,20,21]))
            print(f"    S11: " + "  ".join(f"@{f}={at(fr,s11,f):.1f}" for f in [18,19,20,21,22]))
